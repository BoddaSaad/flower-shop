<header class="mb-10">
    <nav class="flex items-start md:items-center justify-between container mx-auto py-4">
        <div class="hidden md:flex items-center gap-4">
            <a href="https://www.instagram.com/calla.flo" target="_blank">
            ...
            <a href="https://www.tiktok.com/@calla.flo" target="_blank">
                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="size-6" viewBox="0 0 16 16">
                    <path d="M9 0h1.98c.144.715.54 1.617 1.235 2.512C12.895 3.389 13.797 4 15 4v2c-1.753 0-3.07-.814-4-1.829V11a5 5 0 1 1-5-5v2a3 3 0 1 0 3 3z"/>
                </svg>
            </a>
            <a href="https://wa.me/966599917787" target="_blank">
        ...
        <div class="md:hidden">
            <button class="cursor-pointer hs-collapse-toggle py-2 px-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 focus:outline-hidden focus:bg-slate-200 disabled:opacity-50 disabled:pointer-events-none" id="hs-basic-collapse" aria-expanded="false" aria-controls="hs-basic-collapse-heading" data-hs-collapse="#hs-basic-collapse-heading">
                <x-heroicon-o-bars-3 class="size-6"/>
            </button>
        </div>
        <div>
            <a href="{{ route('home') }}">
                <img src="{{ asset('Calla-Logo.png') }}" alt="{{ config('app.name') }}" class="h-12">
            </a>
        </div>
    ...
    <nav class="hidden md:block py-3 bg-slate-100">
    ...
    <div id="hs-basic-collapse-heading" class="hs-collapse hidden w-full overflow-hidden transition-[height] duration-300" aria-labelledby="hs-basic-collapse">
        <div class="border-s-2 border-slate-200">
            <nav class="-ms-0.5 flex flex-col space-y-3">
                @foreach($navCategories as $category)
                    <a href="{{ route('products.index', ['filter[category]' => $category->id]) }}" class="py-1 ps-4 inline-flex items-center gap-2 border-s-2 border-transparent hover:border-slate-600 whitespace-nowrap text-slate-500 hover:text-slate-600 focus:outline-hidden focus:text-slate-600">
                        {{ $category->name }}
                    </a>
                @endforeach
            </nav>
        ...
        <div class="flex items-center justify-center gap-4">
            ...
            <a href="https://wa.me/966599917787" target="_blank">
                <svg xmlns="http://www.w3.org/2000/svg" class="size-5" fill="none" viewBox="0 0 16 16"><path fill="#000" fill-rule="evenodd" d="M11.384 9.489c-.186-.093-1.099-.542-1.269-.604s-.294-.093-.418.093c-.124.185-.48.604-.588.728s-.217.14-.402.046c-.186-.093-.784-.289-1.494-.922A5.6 5.6 0 0 1 6.18 7.544c-.108-.186-.012-.287.081-.38.084-.082.186-.216.279-.325.093-.108.124-.186.186-.31.062-.123.03-.232-.016-.325s-.418-1.007-.572-1.379c-.151-.362-.304-.313-.418-.319a8 8 0 0 0-.356-.006.68.68 0 0 0-.495.232c-.17.186-.65.636-.65 1.55s.665 1.797.758 1.92c.093.125 1.31 2 3.173 2.805.443.192.789.306 1.058.391.445.142.85.122 1.17.074.357-.053 1.099-.45 1.254-.883.155-.434.155-.806.108-.883-.046-.078-.17-.124-.356-.217m-3.389 4.627h-.002a6.17 6.17 0 0 1-3.145-.861l-.225-.134-2.338.613.624-2.28-.147-.233a6.16 6.16 0 0 1-.945-3.288 6.187 6.187 0 0 1 6.18-6.178c1.65.001 3.202.644 4.369 1.812a6.14 6.14 0 0 1 1.807 4.37 6.187 6.187 0 0 1-6.178 6.179M13.253 2.68A7.39 7.39 0 0 0 7.995.5C3.898.5.564 3.834.562 7.932c0 1.31.342 2.59.992 3.716L.5 15.5l3.94-1.034a7.4 7.4 0 0 0 3.552.905h.003c4.097 0 7.432-3.334 7.433-7.433a7.4 7.4 0 0 0-2.175-5.258" clip-rule="evenodd"></path></svg>
            </a>
        </div>
    </div>
    <div class="md:hidden w-full bg-white border-t shadow-[0_-5px_10px_rgba(0,0,0,0.2)] fixed bottom-0 flex justify-between items-center gap-2">
        @auth()
